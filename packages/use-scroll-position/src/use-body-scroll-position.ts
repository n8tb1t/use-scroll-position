...
import { useRefs } from './use-window-scroll-position'
...
const get = (
  ...
  if (!HAS_BROWSER || !root.current) {
    return {
      position: { x: 0, y: 0 },
      properties: { top: false, bottom: false, rect: null }
    }
  }
  ...
  const rect = root.current.document.body.getBoundingClientRect()

  const top = rect.top >= 0

  const bottom =
    document.documentElement.scrollHeight - viewH - rect.top * -1 <= 0
...
export type UseBodyScrollPositionCallbackOptions =
 ...
 */
export const useBodyScrollPosition = scrollPositionFactory(get, useRefs)
